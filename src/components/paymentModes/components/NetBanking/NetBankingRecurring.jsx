...
function NetbankingRecurring({ displayOrderNo, eMandateNo, maxAmountLimit, parentOrderNo, order_id, cust_id, mid, expire_at, orderNo, productId }) {
  ...
  //  useEffect(() => {
  //  const rUrl =  getDomain() + `/pay/payment/thankyou/emandate`;  //-->to be updated @aniket
  ...
  const setBankCode = async (ifscCode) => {
    try {
      const url = `https://ifsc.razorpay.com/${ifscCode}`; // to be updated @aniket
  ...
  const handleChange = (event) => {
    //  console.log('bankId: ' + event.target.value);
  ...
  //     const netBankingMethod = payModesToPass.find(method => method.code === 'EMANDATE_NB');
  ...
  useEffect(() => {
    ...
    if (netBankingMethod && netBankingMethod.subPayModes) {
      setBankList(netBankingMethod.subPayModes); 
    }
  }, []);
  ...
  const handlePayNow = async () => {
    ...
    if (isValid) {
      try {
        const response = await fetch('/pay/recurring/gateway', {  //to be updated @aniket
          method: 'POST',
          headers: {
        ...
        const bankName = getPgCodeByBankId(selectBank);
        ...
        if (gatewayInResponse === 'hdfcEmandate') {
          ...
          //  form.action = url; //'http://localhost:9080/pay/eMandate/process';  //to be updated @aniket
          ...
          const jsonData={
            ...
            ifscCode:ifscCode,
            accoutnHolderName:accHolderName,
            bank:bankFullName,
            method:'Net',
          ...
          try {
            const response = await fetch('/pay/eMandate/update', { // to be updated @aniket
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
          ...
          const options = {
            ...
            handler: function (response) {
              responseInputRef.current.value =
                `razorpay_order_id=${response.razorpay_order_id}&razorpay_payment_id=${response.razorpay_payment_id}&eMandateNo=${eMandateNo}`; // to be updated @aniket
  ...
  return (
    <div className='emandate-card-details'>
      ...
      <div className="note-msg">
        <strong>Note</strong>: Future payments on this account will be charge automatically.
      </div>
      ...
      <div className='text-right'>
        <Button className="register-autopay-btn w-full" onClick={handlePayNow} sx={{ width: { xs: '100%', sm: 'auto' } }}>Register Autopay</Button>
      </div>
...
export default NetbankingRecurring;
