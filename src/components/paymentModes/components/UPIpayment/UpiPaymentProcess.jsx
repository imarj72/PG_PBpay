...
import { Button, Grid, Box, Typography, Paper, Alert, CircularProgress } from '@mui/material';
...
function UpiPaymentProcess({ QrCodeData, handleCloseDialog, vpa }) {
  const SessionTimer = () => {
    const mockEndTime = new Date().getTime() + 100000; 
    const [remainingTime, setRemainingTime] = useState(calculateRemainingTime(mockEndTime));
    ...
    useEffect(() => {
      const timer = setInterval(() => {
        setRemainingTime(calculateRemainingTime(mockEndTime));
      }, 1000);
    ...
    return (
      <Box position="relative" display="flex" alignItems="center" justifyContent="center" gap={1} sx={{ marginTop: '20px', marginBottom: '20px' }}>
