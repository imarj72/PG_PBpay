...
import { FormControl, InputLabel,Select, MenuItem,TextField, FormGroup, FormControlLabel,Button,Checkbox,Dialog } from '@mui/material';
...
function UpiDetailsEmandate({payMode=mockData.payMode, productId=mockData.productId, txAmount=mockData.txAmount, showEmanQrCode=mockData.showEmanQrCode}) {
  ...
  const getHdfcEmanStatuResponse = async (eMandateNo, orderId, payId)=>{
    const params = new URLSearchParams({
      eMandateNo: eMandateNo,
      orgPspRefNo: orderId,
      payId: payId
    });
    const url = `/pay/eMandate/hdfc/emanStatus?${params.toString()}`;
    const responseGateWayValidation = await fetch(url, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
...
// }
  ...
  const handleQRPayNow = async () => {
    ...
    try {
      ...
      if(data && data.decQrPayId){
       ...
       setTimeout(() => checkQRTransactionStatus(decPayId, var1, var2,eMandateNo), data.qrCodeWaitStatusTime * 1000);
  ...
  const submitFormWithQR = (payId, var1, var2) => {
  ...
  return (
    ...
    <div className='upi-emandate-box'>
     {showUpiQR && !isMobile && (
        <>
            <div className="upi-head1 clearfix1">
              <div className="left upi-gateway-warpper ">
                Pay &amp; Register Autopay using QR Code
                <p className="small-text">Click to view &amp; scan the QR code
                  with your prefered UPI app</p>
              </div>
            </div>
           <div className="qrCodeWrapper" id="EmanQrCodeBtn" 
      ...
      {upiProviderId!==-1 && (
        ...
        <p className="upi-provider-inner">
  ...
  <div>
    ...
    <div 
      ...
      style={{
        width: '100%',
        height: '229px',
        background: 'url(images/qr-wrapper.svg) no-repeat center',
        cursor: 'pointer'
      }}
      onClick={() => handleQRPayNow()}
    ></div>
...
export default UpiDetailsEmandate;
